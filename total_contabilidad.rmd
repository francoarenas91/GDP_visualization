...
flux %>% 
  ...
  mutate(name = if_else(str_detect(name, "Transferencias de capital"), "Transferencias de capital", name)) %>% 
    select(code,name,type,account,direction,values_2022,RM,color)->flux
...
values_RoW %>% 
  ...
  left_join(RoW_accounts %>% select(code,type,account,color),by=c("code","type")) %>% 
  mutate(name=case_when(
...
sankey <- sankeyNetwork(Links = links, 
                        ...
                        sinksRight=F,
                        nodePadding = 10,
                        LinkGroup = "color", #agrupar links para colores, sin grupo en gris
                        NodeGroup = "grupos", #agrupar nodos para colores, sin grupo gris
